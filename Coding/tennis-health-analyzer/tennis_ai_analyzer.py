...
class TennisAIAnalyzer:
    ...
    def generate_ai_analysis(self, workout_raw_data: Dict) -> str:
        ...
        try:
            # The practical prompt provided by user
            system_prompt = "你是一位专业且务实的网球战术教练。请基于客观的生理数据，为我制定高效的比赛策略。"
            
            user_prompt = f"""# Style Guidelines
- **语言风格**：平实、直接、客观。
- **核心要求**：拒绝废话，直接指出问题本质，提供可操作的建议。
- **篇幅限制**：全文字数控制在 200 字以内。

# Context
我上传了一段 Apple Watch 网球运动数据。

# Task
第一步：数据有效性检查
如果 `duration`（时长）< 10 分钟 或 `avgHeartRate`（平均心率）< 70 bpm，请直接输出：“**数据无效：时长过短或强度不足，无法进行战术分析。**” 并结束回答。如果数据有效，请不要输出任何关于数据有效性的确认信息，直接跳至第二步。
...
1. **体能状态评估**：
   * 客观描述当前的体能储备情况
2. **关键生理瓶颈**：
   * 结合数据指出几个最影响竞技表现的具体问题。
3. **战术调整建议**：
   * 基于上述情况，给出具体的打法指导。
   * **核心逻辑**：针对体能现状，提供具体的网球技战术方案。
